...
try: 
    ...
    raise ConnectionError("i\aCannot verify connection to yeetssite(https://yeetssite.github.io)\nCheck your internet connection, dipshit.")
    sys.stdout.write("[0m")
    sys.stdout.flush()
...
altDev = True
versionForkID = "a"
versionFork = "yeetssite/asciiFarter"
versionFloat = 1.3
...
if '-v' in args or '--version' in args:    
    ...
    if fartVerFloat > versionFloat:
        ...
        if altDev:
            print("This was an alternate development version:\n Fork: " + versionFork)
        verWarning()
    elif fartVerFloat == versionFloat:
        print("Up to date. (" + fartVer + " same as " + version + ")")
        if altDev:
            print("This is an alternate fork version from: " + versionFork + ".")
        verWarning()
    elif fartVerFloat < versionFloat:
        print("Developing, are we? (" + fartVer + " under " + version + ")")
        if altDev:
            print("\a\x1b[33mYou are on an alternate development fork: " + versionFork + ". Features, facts and glitches seen in the main version may not apply here.\x1b[0m")
        verWarning()
